...
pub fn sanitize_path<P: AsRef<Path>>(path: P) -> Result<PathBuf, AppError> {
    ...
    if path.components().any(|c| matches!(c, std::path::Component::ParentDir)) {
        return Err(AppError::ConfigError("Path traversal detected (..) ".to_string()));
    }

    // Canonicalize if exists, otherwise just return as is but validated
    if path.exists() {
        Ok(dunce::canonicalize(path).map_err(|e| AppError::ConfigError(format!("Failed to canonicalize path: {}", e)))?)
    } else {
        Ok(path.to_path_buf())
    }
}
...
pub fn check_file_size(path: &Path, max_mb: u64) -> Result<(), AppError> {
    ...
    if metadata.len() > max_mb * 1024 * 1024 {
        return Err(AppError::ParseError(format!("File too large ({} MB), maximum allowed is {} MB. Use streaming mode.", 
            metadata.len() / 1024 / 1024, max_mb)));
    }
    Ok(())
}
