...
fn test_scenario_1_insider_pivot() {
    ...
    if let Some(a) = engine.check_acl("666 666 666") { alerts.push(a); }
    if let Some(a) = engine.check_working_hours(chrono::Utc::now()) { alerts.push(a); }
...
fn create_mock_config() -> AppConfig {
    AppConfig {
        app: AppSettings { name: "Test".into(), log_level: "info".into(), report_output_dir: "reports".into() },
        anydesk: AnyDeskSettings { trace_path: "".into(), system_conf_path: "".into(), service_conf_path: "".into(), svc_trace_path: "".into() },
        ...
        reporting: ReportingSettings { format: "json".into(), enable_syslog: false, syslog_server: "".into() },
    }
}
